...
bool memory_init(MemoryContext *memory, const MemoryConfig *config) {
  ...
  if (!arena_create_checked(&memory->arenas[MEMORY_ARENA_PERMANENT],
                            config->permanent_size)) {
    LOG_ERROR("Failed to create permanent arena");
    goto fail;
  }

  if (!arena_create_checked(&memory->arenas[MEMORY_ARENA_TRANSIENT],
                            config->transient_size)) {
    LOG_ERROR("Failed to create transient arena");
    goto fail;
  }

  if (!arena_create_checked(&memory->arenas[MEMORY_ARENA_FRAME],
                            config->frame_size)) {
    LOG_ERROR("Failed to create frame arena");
    goto fail;
  }

  if (!arena_create_checked(&memory->arenas[MEMORY_ARENA_SCRATCH],
                            config->scratch_size)) {
