...
void *arena_alloc_aligned(Arena *arena, size_t size, size_t alignment)
{
  if(alignment == 0 || !arena_is_power_of_two(alignment)) {
    LOG_ERROR(
      "Arena alignment must be a non-zero power of two (got %zu)", alignment);
    return NULL;
  }
  ...
  if(arena_add_overflow(arena->pos, alignment - 1, &pos_plus)) {
    LOG_ERROR(
      "Arena alignment overflow: pos %zu, alignment %zu", arena->pos,
      alignment);
    return NULL;
  }
  ...
  if(aligned_pos > arena->size || size > arena->size - aligned_pos) {
    LOG_ERROR(
      "Arena out of memory: need %zu bytes, have %zu bytes", size,
      arena->size - aligned_pos);
    return NULL;
  }
...
void arena_scratch_init(size_t size)
{
  if(g_scratch_arena && g_scratch_arena != &g_scratch_storage) {
    LOG_WARN(
      "Scratch arena already set externally; reinitializing storage scratch");
  }
