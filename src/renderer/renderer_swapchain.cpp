...
VkResult renderer_internal_create_framebuffers(Renderer *renderer) {
  ...
  for(u32 i = 0; i < renderer->framebuffer_count; i++) {
    ...
    VkResult vk_result = vkCreateFramebuffer(renderer->device.device, &fb_info, NULL, &renderer->framebuffers[i]);
...
void renderer_internal_destroy_framebuffers(Renderer *renderer) {
  for(u32 i = 0; i < renderer->framebuffer_count; i++) {
    if(renderer->framebuffers[i] != VK_NULL_HANDLE) {
      vkDestroyFramebuffer(renderer->device.device, renderer->framebuffers[i], NULL);
    }
  }
...
VkResult renderer_internal_create_render_finished(Renderer *renderer) {
  ...
  for(u32 i = 0; i < renderer->swapchain.image_count; i++) {
    VkResult vk_result
      = vkCreateSemaphore(renderer->device.device, &semaphore_info, NULL, &renderer->render_finished[i]);
    if(vk_result != VK_SUCCESS) {
      for(u32 j = 0; j < i; j++) {
        vkDestroySemaphore(renderer->device.device, renderer->render_finished[j], NULL);
...
void renderer_internal_destroy_render_finished(Renderer *renderer) {
  for(u32 i = 0; i < renderer->swapchain.image_count; i++) {
    if(renderer->render_finished[i] != VK_NULL_HANDLE) {
      vkDestroySemaphore(renderer->device.device, renderer->render_finished[i], NULL);
...
VkResult renderer_internal_recreate_swapchain(Renderer *renderer, u32 width, u32 height) {
  ...
  VkResult vk_result = vk_swapchain_recreate(&renderer->device, renderer->surface, width, height, &renderer->swapchain);
  if(vk_result != VK_SUCCESS) {
    return vk_result;
  }
