...
Result renderer_create(Renderer **out_renderer, WindowContext *window,
  ...
  *out_renderer = NULL;
  ...
  vk_result = window_create_surface(window, renderer->instance.instance,
                                    &renderer->surface);
  if (vk_result != VK_SUCCESS) {
  ...
  vk_result = vk_device_create(renderer->instance.instance, renderer->surface,
                               &renderer->device);
  if (vk_result != VK_SUCCESS) {
  ...
  vk_result =
      vk_command_create(renderer->device.device,
                        renderer->device.queue_families.graphics_family,
                        &renderer->command);
  if (vk_result != VK_SUCCESS) {
  ...
  vk_result = vk_renderpass_create(renderer->device.device,
  ...
  vk_result =
      vk_pipeline_create(renderer->device.device,
