...
Result renderer_draw(Renderer *renderer)
{
  ...
  if(renderer->images_in_flight[image_index] != VK_NULL_HANDLE) {
    vk_result = vkWaitForFences(
      renderer->device.device, 1, &renderer->images_in_flight[image_index],
      VK_TRUE, UINT64_MAX);
    if(vk_result != VK_SUCCESS) {
  ...
  vkCmdBindPipeline(
    cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer->pipeline->pipeline);
  ...
  vk_result = vkQueueSubmit(
    renderer->device.graphics_queue, 1, &submit_info, frame->in_flight);
  if(vk_result != VK_SUCCESS) {
