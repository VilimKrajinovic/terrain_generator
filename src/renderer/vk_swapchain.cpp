  ...
  VkPhysicalDevice device, VkSurfaceKHR surface, Arena *arena)
{
  ...
  vkGetPhysicalDeviceSurfaceFormatsKHR(
    device, surface, &details.format_count, NULL);
  if(details.format_count > 0) {
    details.formats
      = ARENA_PUSH_ARRAY(arena, VkSurfaceFormatKHR, details.format_count);
    vkGetPhysicalDeviceSurfaceFormatsKHR(
  ...
  if(details.present_mode_count > 0) {
    details.present_modes
      = ARENA_PUSH_ARRAY(arena, VkPresentModeKHR, details.present_mode_count);
    vkGetPhysicalDeviceSurfacePresentModesKHR(
...
choose_surface_format(const VkSurfaceFormatKHR *formats, u32 count)
{
...
choose_present_mode(const VkPresentModeKHR *modes, u32 count)
{
...
static VkResult create_image_views(VkDevice device, VkSwapchainContext *ctx)
{
  for(u32 i = 0; i < ctx->image_count; i++) {
    ...
    VkResult result
      = vkCreateImageView(device, &create_info, NULL, &ctx->image_views[i]);
  ...
  VkSwapchainKHR old_swapchain, VkSwapchainContext *ctx)
{
  ...
  VkSurfaceFormatKHR surface_format
    = choose_surface_format(support.formats, support.format_count);
  VkPresentModeKHR present_mode
    = choose_present_mode(support.present_modes, support.present_mode_count);
  ...
  LOG_DEBUG(
    "Swapchain format: %d, present mode: %d, image count: %u",
  ...
  VkResult result = vkCreateSwapchainKHR(
    device->device, &create_info, NULL, &ctx->swapchain);
  ...
  vkGetSwapchainImagesKHR(
    device->device, ctx->swapchain, &ctx->image_count, NULL);
  if(ctx->image_count > MAX_SWAPCHAIN_IMAGES) {
    ctx->image_count = MAX_SWAPCHAIN_IMAGES;
  }
  vkGetSwapchainImagesKHR(
    device->device, ctx->swapchain, &ctx->image_count, ctx->images);
  ...
  VkSwapchainContext *ctx)
{
  ...
  VkResult result
    = vk_swapchain_create(device, surface, width, height, old_swapchain, ctx);
